...
export const fetchAnimeList = async ({ search = '', page = 1, limit = 20 } = {}) => {
...
export const fetchAnimeRecommendations = async (id, limit = 6) => {
  const res = await fetch(`${API_BASE}/anime/${id}/recommendations?limit=${limit}`);
  if (!res.ok) throw new Error(`Failed to fetch recommendations for anime #${id}`);
  return res.json();
};
