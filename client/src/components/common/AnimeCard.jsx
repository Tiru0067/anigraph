  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-2/3 w-full overflow-hidden rounded-md bg-background-200 border border-background-400/25 group-hover:border-primary-400/60 transition-colors">
        ...
        {anime.averageScore ? (
          <div className="absolute top-1 right-1 px-1.5 py-0.5 rounded bg-background-000/80 backdrop-blur-md border border-white/10 flex items-center gap-0.5 text-[10px] font-bold text-amber-300">
            <Star className="w-2.5 h-2.5 fill-amber-300 mr-px" />
            <span>{(anime.averageScore / 10).toFixed(1)}</span>
          </div>
        ) : null}

        {/* Hover Quick Action Overlay */}
        <div className="absolute inset-0 bg-linear-to-t from-background-000/85 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex flex-col justify-end p-2">
          <span className="text-[10.5px] font-medium text-primary-200 flex items-center gap-1">
