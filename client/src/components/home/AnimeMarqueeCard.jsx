...
export const AnimeMarqueeCard = ({ anime, uniqueKey }) => {
  ...
  return (
    <Link
      to={`/anime/${anime.id}`}
      key={uniqueKey}
      className="group relative flex-shrink-0 w-32 sm:w-36 md:w-40 aspect-[2/3] rounded-xl overflow-hidden bg-background-200 border border-background-400/30 hover:border-primary-400/60 shadow-md hover:shadow-xl hover:shadow-primary-500/15 transition-all duration-300 hover:-translate-y-1"
    >
      <img
        src={cover}
      ...
      {anime.averageScore && (
        <div className="absolute top-1.5 right-1.5 px-1.5 py-0.5 rounded bg-background-000/75 backdrop-blur-md border border-white/10 flex items-center gap-0.5 text-[11px] font-bold text-amber-300">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-background-000 via-background-000/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-2.5 text-left">
        ...
        <p className="text-[12px] font-bold text-typography-100 line-clamp-2 leading-tight">
          {anime.titleEnglish || anime.titleRomaji}
        </p>
        <div className="flex items-center justify-between text-[10px] text-typography-300 mt-1">
